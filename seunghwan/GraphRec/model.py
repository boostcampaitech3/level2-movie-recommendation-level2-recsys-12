...
class GraphRecModel(nn.Module) :
    ...
    def forward(self, user_input, movie_input) :
        ...
        user_mf = np.concatenate(F.relu(user_mf), F.relu(-user_mf), axis=1)
        movie_mf = self.movie_layer(movie_input)
        movie_mf = np.concatenate(F.relu(movie_mf), F.relu(-movie_mf), axis=1)
        
        input = torch.multiply(user_mf, movie_mf)
        pred = torch.sum(input, dim=1)
