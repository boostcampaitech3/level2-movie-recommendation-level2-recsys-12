...
class Data:
    ...
    def create_splits(self, n, k_test, shuffle=False, n_fold=True, generators=True, batch_size=1024):
        """
...
class Split:
    ...
    def __init__(self, data, k_test, shuffle=False, index=0, generator=True, batch_size=1024):
        """
        ...
        self.inference_users = self.all_users
        if generator:
    ...
    def generators(self,
        ...
        self.inference_gen = SplitGenerator(
            data_df=self.inference_purchases_txt(),
            itemizer=self.master_data.toki,
            batch_size=128,
            random_batching=False,
            prevent_identity=False,
    ...
    def inference_purchases_txt(self):
        return self.master_data.purchases_txt.copy(deep=True)
...
class Evaluator:
    ...
    def __init__(self, split, method='leave_random_20_pct_out', data="test", debug=False):
        ...
        if data == "val":
            ...
            self.ivx = split.inference_gen.data.set_index(split.inference_gen.data.userid).sort_index().itemids.apply(
                lambda x: x.split(','))
        else:
